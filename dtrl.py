...
def list_triangles(G):
    ...
    #   examples.triangle_type(2) (which means the triangle is either Type I or Type II) but not to the graph   
    #   examples.triangle_type(1).
    
    H=deepcopy(G)
    S=[set(),set(),set(),set()]
    
    for triangle in H.subgraph_search_iterator(examples.triangle_type(1)):
        triangle_vertices=extract_triangle_vertices(triangle)
        S[0].add(triangle_vertices)
        
    for triangle in H.subgraph_search_iterator(examples.triangle_type(2)):
        triangle_vertices=extract_triangle_vertices(triangle)
        if triangle_vertices not in S[0]:
            S[1].add(triangle_vertices)
            
    for triangle in H.subgraph_search_iterator(examples.triangle_type(3)):
        triangle_vertices=extract_triangle_vertices(triangle)
        if triangle_vertices not in S[0]:
            S[2].add(triangle_vertices)
            
    for triangle in H.subgraph_search_iterator(examples.triangle_type(4)):
        triangle_vertices=extract_triangle_vertices(triangle)
